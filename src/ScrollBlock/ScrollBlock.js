...
import style from './ScrollBlock.module.css';
...
componentDidMount() {
    ...
    BlockScrolling = () => {
        let scrollBlockRef =this.scrollBlockRef.current;

        let scrollBlockHeight =this.scrollBlockRef.current.getBoundingClientRect().height;
        // let scrollBlockTop =this.scrollBlockRef.current.getBoundingClientRect().top;

        let containerTop =this.Container.current.getBoundingClientRect().y;
        let containerHeight =this.Container.current.getBoundingClientRect().height;

        let containerBottom = window.pageYOffset+containerTop+containerHeight-scrollBlockHeight;

        let scrollBlockPositionRelative = containerHeight-scrollBlockHeight;

        if (window.pageYOffset <= window.pageYOffset+ containerTop) {
// Прилипание блока к началу контейнера
            scrollBlockRef.style.cssText = `
                position: relative;
...
// Прилипание блока к началу контейнера
            scrollBlockRef.style.cssText = `
                position: relative;
                top:${scrollBlockPositionRelative}px;
        `;
        } else {
// Если страницу прокрутили дальше, чем находится блок, то блок фиксируется и отображ. сверху
            scrollBlockRef.style.cssText = `
                position: fixed;
        `;
        }
    ...
    render() {
        ...
        return (
            <div className={style.Container}>
                ...
                <div className={style.Container1} ref={this.Container}>
                    <aside className={style.RightAside} ref={this.scrollBlockRef} >RightAside
